          ...
          <nav className="hidden md:flex items-center gap-6 text-[18px] lg:text-[17px]">
            {translatedLinks.map((link, idx) => (
              <Link
                key={idx}
                href={link.path}
                className={`hover:text-green-600 transition-colors duration-200 ${pathname === link.path ? "font-medium text-green-600" : ""
                  }`}
              >
                {link.label}
              </Link>
                ...
                <div className="absolute right-0 mt-2 w-40 bg-white rounded-md shadow-lg py-1 z-50 border border-gray-200">
                  {languages.map((lang) => (
                    <Button
                      ...
                      className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-50 ${currentLocale === lang.code ? "text-green-600 font-medium" : "text-gray-700"
                        }`}
                    >
                      {lang.name}
                    </Button>
            ...
            <nav className="flex flex-col gap-4 text-[17px]">
              {translatedLinks.map((link, idx) => (
                <Link
                  key={idx}
                  href={link.path}
                  className={`py-2 px-3 rounded-lg hover:bg-gray-50 ${pathname === link.path ? "text-green-600 font-medium" : ""
                    }`}
                  ...
                  <div className="pl-8 py-1">
                    {languages.map((lang) => (
                      <button
                        key={lang.code}
                        onClick={() => handleChangeLocale(lang.code)}
                        className={`block w-full text-left py-1.5 px-2 text-sm rounded ${currentLocale === lang.code
